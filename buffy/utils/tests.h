#ifndef BUFFY_UTILS_TESTS_H
#define BUFFY_UTILS_TESTS_H
 ...
 */
extern const buffy::utils::tests::LocationInfo buffy_utils_test_location_info;
 ...
 */
#define BUFFY_UTILS_TEST_INFO(name) \
    buffy::utils::tests::LocationInfo buffy_utils_test_location_info; \
    buffy::utils::tests::LocationInfo& name = buffy_utils_test_location_info
...
#define wassert(...) \
    do { try { \
        __VA_ARGS__ ; \
    } catch (buffy::utils::tests::TestFailed& e) { \
        e.add_stack_info(__FILE__, __LINE__, #__VA_ARGS__, buffy_utils_test_location_info); \
        throw; \
    } catch (std::exception& e) { \
        throw buffy::utils::tests::TestFailed(e, __FILE__, __LINE__, #__VA_ARGS__, buffy_utils_test_location_info); \
    } } while(0)
...
#define wcallchecked(func) \
    [&]() { try { \
        ...
        e.add_stack_info(__FILE__, __LINE__, #func, buffy_utils_test_location_info); \
        throw; \
    } catch (std::exception& e) { \
        throw buffy::utils::tests::TestFailed(e, __FILE__, __LINE__, #func, buffy_utils_test_location_info); \
    } }()

/**
 * Fail a test with an error message
 */
#define wfail_test(msg) wassert(throw buffy::utils::tests::TestFailed((msg)))
